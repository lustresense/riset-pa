% ============================================================
% DRAF PROPOSAL PROYEK AKHIR — FARCHAN DEANO MUHAMMAD (CAN)
% Versi: v0.0.0
...
% 
% Compiled dari:
%   - bab1_can_v1.tex (Bab 1 verbatim, tidak diubah)
%   - bab2_can_v1.tex (Bab 2 ringkas, hapus MediaPipe, ganti Children Behavioral Patterns)
%   - bab3_can_v1.tex (Bab 3 detail, pivot 16/6/26 applied)
%   - daftar_pustaka_can_v1.tex (32 refs)
% 
% Post-pivot 16/6/26 applied:
%   #1 Login: nomor absen + superadmin (BUKAN gesture MediaPipe)
%   #2 Override: Top-6 check (BUKAN Override Budget)
%   #3 Timer count-up + time_spent_ms
%   #4 Resize/Rotate buttons UI (NOT slider, NOT gesture)
%   #5 Hapus Decorative category (Solid/Danger only)
% 
% CATATAN: Bab 1 masih mention MediaPipe (verbatim per arahan user).
%          Pertimbangkan update di iterasi v0.0.1.
% ============================================================

\documentclass[12pt,a4paper,oneside,openany]{report}

% ============================================================
% PACKAGES — PENS Standard
% ============================================================
...
% Placeholder figure macro (POLOSAN, no color) — minimal version untuk hemat halaman
\newcommand{\placeholderfig}[2]{%
  \begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering\vspace{0.4cm}\textit{\small[Placeholder: #1 — akan dilengkapi pada tahap implementasi]}\vspace{0.4cm}}}
    \caption{#2}
    \label{fig:#2}
  \end{figure}
}
...
\thispagestyle{empty}
\begin{center}
\textbf{PROPOSAL PROYEK AKHIR}\\[1cm]
\textbf{}\\[0.5cm]
\textbf{SKETCHBOOK UNIVERSE: SIMULASI INTERAKTIF BERBASIS HUMAN-IN-THE-LOOP UNTUK LITERASI KECERDASAN BUATAN SISWA SMP}\\[0.5cm]
\textbf{SKETCHBOOK UNIVERSE: INTERACTIVE SIMULATION BASED ON HUMAN-IN-THE-LOOP FOR AI LITERACY OF JUNIOR HIGH SCHOOL STUDENTS}\\[2cm]
\textbf{Diajukan Untuk Memenuhi Salah Satu Syarat Memperoleh Gelar Sarjana Teknik}\\[0.5cm]
\textbf{pada Program Studi D4 Teknologi Rekayasa Multimedia}\\[0.5cm]
\textbf{Politeknik Elektronika Negeri Surabaya}\\[2cm]
\textbf{Farchan Deano Muhammad}\\[0.3cm]
\textbf{NRP: 5322600011}\\[2cm]
\textbf{Dosen Pembimbing:}\\[0.3cm]
\textbf{[Nama Dosen Pembimbing]}\\[2cm]
\textbf{POLITEKNIK ELEKTRONIKA NERI SURABAYA}\\[0.3cm]
\textbf{TAHUN 2026}
\end{center}
\end{titlepage}
...
\thispagestyle{empty}
\begin{center}
\textbf{PROPOSAL PROYEK AKHIR}\\[0.5cm]
\textbf{SKETCHBOOK UNIVERSE: SIMULASI INTERAKTIF BERBASIS HUMAN-IN-THE-LOOP UNTUK LITERASI KECERDASAN BUATAN SISWA SMP}\\[1cm]
\end{center}
Diajukan oleh:
\begin{itemize}
\item \textbf{Nama:} Farchan Deano Muhammad
\item \textbf{NRP:} 5322600011
\item \textbf{Program Studi:} D4 Teknologi Rekayasa Multimedia
\end{itemize}
Telah disetujui dan disahkan sebagai Proposal Proyek Akhir di Politeknik Elektronika Negeri Surabaya pada tanggal \underline{\hspace{2cm}}.
\vspace{1cm}
\begin{flushright}
Surabaya, \underline{\hspace{3cm}}\\[1cm]
Dosen Pembimbing,\\[2cm]
\underline{\hspace{5cm}}\\[0.3cm]
\textbf{[Nama Dosen Pembimbing]}
\end{flushright}
\newpage
...
\thispagestyle{empty}
\textbf{SKETCHBOOK UNIVERSE: SIMULASI INTERAKTIF BERBASIS HUMAN-IN-THE-LOOP UNTUK LITERASI KECERDASAN BUATAN SISWA SMP}\\[0.5cm]
\textbf{Nama:} Farchan Deano Muhammad \quad \textbf{NRP:} 5322600011\\[1cm]
Kecerdasan buatan (AI) telah menjadi bagian tak terpisahkan dari kehidupan sehari-hari, termasuk bagi siswa Sekolah Menengah Pertama (SMP). Namun, paparan terhadap AI tanpa scaffolding yang tepat berisiko menimbulkan automation bias, yaitu kecenderungan menerima output AI secara otomatis tanpa evaluasi kritis. Penelitian ini mengusulkan Sketchbook Universe, sebuah simulasi interaktif berbasis Human-in-the-Loop (HITL) yang dirancang untuk memfasilitasi interaksi reflektif siswa SMP terhadap output AI. Sistem ini mengintegrasikan canvas drawing, klasifikasi sketsa berbasis Convolutional Neural Network (CNN) MobileNet melalui TensorFlow.js, mekanisme Top-6 check anti-cheat, maskot pedagogical agent Momo berbasis rule-based non-NLP, serta tiga level progression (Trust Build, Ambiguity, Override). Penelitian ini menggunakan metode Mixed Methods dengan pendekatan Research and Development (R\&D), metode pengembangan SDLC Waterfall, dan teknik analisis Fishbone 6M. Pengujian dilakukan menggunakan System Usability Scale (SUS) dan analisis pola perilaku melalui K-Means clustering. Scope penelitian mencakup frontend, UI/UX, maskot Momo, level design, dan mekanisme HITL, sementara backend, CNN, dan K-Means ditangani oleh rekan penelitian sebagai proyek kelompok.\\[1cm]
\textbf{Kata Kunci:} Human-in-the-Loop, Literasi AI, Siswa SMP, Sketch Recognition, Pedagogical Agent, System Usability Scale
\newpage
...
\thispagestyle{empty}
\textbf{SKETCHBOOK UNIVERSE: INTERACTIVE SIMULATION BASED ON HUMAN-IN-THE-LOOP FOR AI LITERACY OF JUNIOR HIGH SCHOOL STUDENTS}\\[0.5cm]
\textbf{Name:} Farchan Deano Muhammad \quad \textbf{NRP:} 5322600011\\[1cm]
Artificial Intelligence (AI) has become an inseparable part of daily life, including for junior high school students. However, exposure to AI without proper scaffolding risks automation bias, the tendency to accept AI output automatically without critical evaluation. This research proposes Sketchbook Universe, an interactive simulation based on Human-in-the-Loop (HITL) designed to facilitate reflective interaction of junior high school students with AI output. The system integrates canvas drawing, sketch classification based on Convolutional Neural Network (CNN) MobileNet via TensorFlow.js, Top-6 check anti-cheat mechanism, Momo pedagogical agent (rule-based non-NLP), and three-level progression (Trust Build, Ambiguity, Override). The research uses Mixed Methods with Research and Development (R\&D) approach, SDLC Waterfall development method, and Fishbone 6M analysis technique. Testing is conducted using System Usability Scale (SUS) and behavioral pattern analysis through K-Means clustering. The research scope covers frontend, UI/UX, Momo mascot, level design, and HITL mechanism, while backend, CNN, and K-Means are handled by the research partner as a group project.\\[1cm]
\textbf{Keywords:} Human-in-the-Loop, AI Literacy, Junior High School Students, Sketch Recognition, Pedagogical Agent, System Usability Scale
\newpage
